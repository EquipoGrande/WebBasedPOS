...
const query_megaQuery = `SELECT * FROM (
    ...
    LEFT JOIN(SELECT sli.productid, sum(sli.quantity) as salequantity FROM salelineitem sli WHERE sli.saleid >= 0 GROUP BY sli.productid) AS s ON c.productid = s.productid 
    LEFT JOIN(SELECT vli.productid, sum(vli.quantity) as salequantity FROM vendorlineitem vli WHERE vli.saleid > 0 GROUP BY vli.productid) AS v ON c.productid = v.productid 
    LEFT JOIN(SELECT pr.productid, pr.productname FROM product pr) AS p ON c.productid = p.productid
    LEFT JOIN(SELECT sli.productid, sum(sli.quantity) as amountSold FROM salelineitem sli WHERE sli.saleid BETWEEN 0 AND 100 GROUP BY sli.productid) AS sl ON c.productid = sl.productid
